	...
	keccakOCL(cl::Context& context, cl::vector<cl::Device>& devices, const char* kernelFile){
		...
		try{
            ...
            program.build(devices, buildOptions.c_str() );
                ...
                d_data = cl::Buffer(context, CL_MEM_READ_ONLY, sizeof(uint64_t)*25);
                d_out = cl::Buffer(context, CL_MEM_WRITE_ONLY, sizeof(uint64_t)*25);
        ...
        inline void initData(cl::CommandQueue& queue, cl::Event& event){
            queue.enqueueWriteBuffer(d_data, CL_TRUE, 0, sizeof(uint64_t)*25, h_data);        
...
#endif
            queue.enqueueReadBuffer(d_out, CL_TRUE, 0, sizeof(uint64_t)*25, h_out);
