...
public:
	keccakOCL(cl::Context& context, cl::vector<cl::Device>& devices, const char* kernelFile, int qntHash){
		...
		try{
			cerr << "buildOptions " << buildOptions << "\nQuantidade de Hash: " << qntHash <<endl;
            program.build(devices, buildOptions.c_str() );
                ...
                h_out = (uint64_t *)malloc(width*sizeof(uint64_t));
                h_data = (uint64_t *)malloc(width*sizeof(uint64_t));
                ...
                d_data = cl::Buffer(context, CL_MEM_READ_ONLY, sizeof(uint64_t)*width);
                d_out = cl::Buffer(context, CL_MEM_WRITE_ONLY, sizeof(uint64_t)*width);
        ...
        inline void initData(cl::CommandQueue& queue, cl::Event& event, int width){
            queue.enqueueWriteBuffer(d_data, CL_TRUE, 0, sizeof(uint64_t)*width, h_data);
        ...
        inline int goldenTest(cl::CommandQueue& queue, cl::Event& event, int width){
            event.wait();
#ifdef PROFILING
...
#endif
            queue.enqueueReadBuffer(d_out, CL_TRUE, 0, sizeof(uint64_t)*width, h_out);
        ...
        cl::NDRange getGlobalWorkItems(int threads){
            return (cl::NDRange(threads));
        }
...
int main(int argc, char** argv) {
    ...
    try{
        ...
        cl::Event event;
        keccak.initData(queue, event, qtdHash*25);

        queue.enqueueNDRangeKernel(keccak.getKernel(),
                                 keccak.getStartRange(),
                                 keccak.getGlobalWorkItems(qtdHash),
                                 keccak.getWorkItemsInWorkGroup(),
                                 NULL,
                                 &event);
        if(keccak.goldenTest(queue, event, qtdHash*25) == 0){
    ...
    printf("\nOutput of GPU: [A]\n");
    for(int i=qtdHash*25-25;i<qtdHash*25;i++) {
